...
```{bash, eval=F, echo=T}
#Extract exons from Gencode annotation
cat gencode.v19.annotation.gtf | grep "exon" | awk -vOFS="\t" '{print $1, $4, $5, $10, $8, $7}' | awk '{gsub(/"|;/,"",$4)}1' | uniq > exons.uniq.bed
awk '{sub(/\..*/,"",$4)} 1' exons.uniq.bed | awk 'sub(/\./,"0",$5)' | awk 'gsub(/^chr/,"",$1)' | tr -s ' ' '\t' | sort -k1,1 -k2,2n > exons.uniq.sorted.bed
#Split into teQTL and non-teQTL
grep -f teqtl.fdr10.names exons.uniq.sorted.bed > teqtl.fdr10.exons.bed
grep -f non.teqtl.fdr10.names exons.uniq.sorted.bed > non.teqtl.fdr10.exons.bed
grep -f non.teqtl.fdr05.names exons.uniq.sorted.bed > non.teqtl.fdr05.exons.bed
grep -f non.teqtl.fdr01.names exons.uniq.sorted.bed > non.teqtl.fdr01.exons.bed
grep -f teqtl.fdr05.names exons.uniq.sorted.bed > teqtl.fdr05.exons.bed
grep -f teqtl.fdr01.names exons.uniq.sorted.bed > teqtl.fdr01.exons.bed
...
bedtools intersect -a psychENCODE.mergedGenotype.RRP.190sample.commonSet.vcf.gz -b non.teqtl.fdr10.exons.bed -wa -wb | bgzip > non.teqtl.fdr10.exons.results.gz
...
```{bash, eval=F, echo=T}
cat gencode.v19.annotation.gtf | grep "intron" | awk -vOFS="\t" '{print $1, $4, $5, $10, $8, $7}' | awk '{gsub(/"|;/,"",$4)}1' | uniq > introns.uniq.bed
awk '{sub(/\..*/,"",$4)} 1' introns.uniq.bed | awk 'sub(/\./,"0",$5)' | awk 'gsub(/^chr/,"",$1)' | tr -s ' ' '\t' | sort -k1,1 -k2,2n > introns.uniq.sorted.bed
#Split into teQTL and non-teQTL
...
```{r}
fdr10.introns <- read.table('introns.results.fdr10.tab')
#Name rows and columns of contingency table
...
```{r}
fdr05.introns <- read.table('introns.results.fdr05.tab')
#Name rows and columns of contingency table
...
```{r}
fdr01.introns <- read.table('introns.results.fdr01.tab')
#Name rows and columns of contingency table
